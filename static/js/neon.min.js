(function(a){var b=function(b,e){var f=a(b),h,c,p,k=a.extend({},{debug:!1,glyphpath:"",meipath:"",bgimgpath:"",bgimgopacity:.6,apiprefix:"",origwidth:null,origheight:null,documentType:"liber",width:1E3},e);a.extend(k,{canvasid:"neon-canvas"});var m=function(b){console.log("loading SVG glyphs ...");return a.get(k.glyphpath,function(c){var d={};a(c).find("svg").each(function(b,c){var f=a("<lol>").append(a(c).clone()).remove().html();fabric.loadSVGFromString(f,function(b,f){var e=a(c).attr("id"),g=fabric.util.groupSVGElements(b,
f);d[e]=new Toe.Model.Glyph(e,g)})});b.setGlyphs(d)})},l=function(){console.log("loading background image ...");var b=a.Deferred();!k.bgimgpath||k.origwidth||k.origheight?b.resolve():fabric.Image.fromURL(k.bgimgpath,function(a){k.origwidth=a.width;k.origheight=a.height;b.resolve()});return b.promise()},g=function(){var b=a.Deferred();k.meipath?a.get(k.meipath,function(a){console.log("loading MEI file ...");h=a;b.resolve()}):b.resolve();return b.promise()},q=function(){var b;switch(k.documentType){case "liber":case "salzinnes":b=
new Toe.Model.SquareNotePage(k.documentType);break;case "stgallen":b=new Toe.Model.CheironomicPage(k.documentType)}var d=a("<canvas>").attr("id",k.canvasid),e=[k.origwidth,k.origheight];k.bgimgpath||(e=b.calcDimensions(a(h).find("zone")));b.setPageScale(k.width/e[0]);b.setDimensions(Math.round(e[0]),Math.round(e[1]));d.attr("width",b.width);d.attr("height",b.height);d.attr("style","border: 4px black solid;");f.prepend(d);d={renderOnAddition:!1};k.bgimgpath&&a.extend(d,{backgroundImage:k.bgimgpath,
...
Toe.Model.System.prototype.getSystemSnapCoordinates=function(a,b,d){var e={ignoreEle:null,x:!0,y:!0};$.extend(e,d);if(e.y){d=this.zone.uly;var f=this.zone.uly+this.delta_y/2,h=(a.y-d)/this.delta_y,c=Math.abs(Math.round(Math.abs(h))-Math.abs(h)),p=(a.y-f)/this.delta_y,k=Math.abs(Math.round(Math.abs(p))-Math.abs(p));Math.min(c,k)==c?a.y=d+Math.round(h)*this.delta_y:a.y=f+Math.round(p)*this.delta_y}if(e.x){d=a.x-b/2;f=a.x+b/2;for(h=0;h<this.elements.length;h++)if(this.elements[h]!=e.ignoreEle&&(c=this.elements[h].zone.ulx,
p=this.elements[h].zone.lrx,!(d>=p))){a.x=d>=c&&d<p||f>=c&&f<p||c>d&&p<f?a.x<c+(p-c)/2?c-b/2:p+b/2:a.x;break}this.elements.length&&this.elements[0]instanceof Toe.Model.Clef&&d<=this.elements[0].zone.lrx?a.x=this.elements[0].zone.lrx+b/2+1:d<=this.zone.ulx&&(a.x=this.zone.ulx+b/2+1);this.custos&&e.ignoreEle!=this.custos&&f>=this.custos.zone.ulx?a.x=this.custos.zone.ulx-b/2-3:f>=this.zone.lrx&&(a.x=this.zone.lrx-b/2-3)}return a};
Toe.Model.System.prototype.getLooseElements=function(){var a=[];if(this.elements&&0<this.elements.length)for(var b=0;b<this.elements.length;b++)(this.elements[b].zone.lrx<this.zone.ulx||this.elements[b].zone.ulx>this.zone.lrx)&&a.push(this.elements[b]);return a};Toe.View.SystemView=function(a){this.rendEng=a;this.drawing=null};Toe.View.SystemView.prototype=new Toe.View.View;Toe.View.SystemView.prototype.constructor=Toe.View.SystemView;
...
getChildren:function(){var a=[],b;for(b in this.children)a.push(this.children[b]);return a}};Toe.View.Interaction=function(a,b,d,e){this.rendEng=a;this.page=b;this.apiprefix=d};Toe.View.Interaction.prototype.constructor=Toe.View.Interaction;Toe.View.Interaction.prototype.showInfo=function(a){$("#info > p").html(a);$("#info").animate({opacity:1},100)};Toe.View.Interaction.prototype.hideInfo=function(){$("#info").animate({opacity:0},100)};Toe.View.Interaction.prototype.showAlert=function(a){$("#alert > p").text(a);$("#alert").animate({opacity:1},100)};
Toe.View.Interaction.prototype.deactivateCanvasObjects=function(){this.rendEng.canvas.selection=!1;this.rendEng.canvas.deactivateAll();this.rendEng.canvas.HOVER_CURSOR=null};Toe.View.Interaction.prototype.activateCanvasObjects=function(){this.rendEng.canvas.selection=!0;this.rendEng.canvas.HOVER_CURSOR="pointer"};Toe.View.Interaction.prototype.removeInsertControls=function(){$("#sidebar-insert").remove()};Toe.View.Interaction.prototype.removeEditControls=function(){$("#sidebar-edit").remove()};
...
Toe.View.NeumeView.prototype.bestDotPlacements=function(a,b,d){var e=[],f=a.zone.uly+a.delta_y/2,h=ypos=b[d],c=Math.round(2*(h-f)/a.delta_y);0==c%2&&e.push(h);var p=ypos-a.delta_y/2,c=Math.round(2*(p-f)/a.delta_y),h=!1;if(0<=d-1&&ypos-a.delta_y/2==b[d-1]||d+1<b.length&&ypos-a.delta_y/2==b[d+1])h=!0;0!=c%2||h||e.push(p);p=ypos+a.delta_y/2;c=Math.round(2*(p-f)/a.delta_y);h=!1;if(d+1<b.length&&ypos+a.delta_y/2==b[d+1]||0<=d-1&&ypos+a.delta_y/2==b[d-1])h=!0;0!=c%2||h||e.push(p);return e};
...
"diamond";break;case Toe.Model.NeumeComponent.Type.punctum_inclinatum_parvum:c="diamond_small";break;case Toe.Model.NeumeComponent.Type.quilisma:c="quilisma"}f.push(this.rendEng.getGlyph(c))}var p=this.rendEng.getGlyph("dot"),k={fixed:[],modify:[]};switch(a.typeid){case "punctum":case "cavum":var c=a.zone.ulx+f[0].centre[0],m=f[0].clone().set({left:c,top:b[0]});k.modify.push(m);if(a.components[0].hasOrnament("dot")){var l=this.bestDotPlacements(d,b,0);k.modify.push(p.clone().set({left:m.left+2*f[0].centre[0],
top:l[0]}))}this.drawLedgerLines([a.rootSystemPos],[c],2*f[0].centre[0],d);break;case "distropha":case "tristropha":var g=[];g.push(a.zone.ulx+f[0].centre[0]);for(var q=0;q<a.components.length;q++)m=f[q].clone().set({left:g[q],top:b[q]}),k.modify.push(m),g.push(g[q]+3*f[q].centre[0]);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var f=e.bestDotPlacements(d,b,a);0<f.length&&k.modify.push(p.clone().set({left:g[g.length-1],top:f[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+
b.pitchDiff}),g,2*f[0].centre[0],d);break;case "virga":c=a.zone.ulx+f[0].centre[0];m=f[0].clone().set({left:c,top:b[0]});k.modify.push(m);a.components[0].hasOrnament("dot")&&(l=this.bestDotPlacements(d,b,0),k.modify.push(p.clone().set({left:m.left+2*f[0].centre[0],top:l[0]})));var n=m.left+f[0].centre[0]-1,n=this.rendEng.createLine([n,b[0],n,b[0]+1.5*d.delta_y],{strokeWidth:2,interact:!0});k.fixed.push(n);this.drawLedgerLines([a.rootSystemPos],[c],2*f[0].centre[0],d);break;case "bivirga":case "trivirga":g=
[];g.push(a.zone.ulx+f[0].centre[0]);for(q=0;q<a.components.length;q++)m=f[q].clone().set({left:g[q],top:b[q]}),k.modify.push(m),n=m.left+f[q].centre[0]-1,n=this.rendEng.createLine([n,b[q],n,b[q]+1.5*d.delta_y],{strokeWidth:2,interact:!0}),k.fixed.push(n),g.push(g[q]+3*f[q].centre[0]);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var f=e.bestDotPlacements(d,b,a);0<f.length&&k.modify.push(p.clone().set({left:g[g.length-1],top:f[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+
b.pitchDiff}),g,2*f[0].centre[0],d);break;case "clivis":g=[];g.push(a.zone.ulx+f[0].centre[0]);var t=f[0].clone().set({left:g[0],top:b[0]});k.modify.push(t);n=g[0]-f[0].centre[0]+1;n=this.rendEng.createLine([n,b[0],n,a.zone.lry],{strokeWidth:2,interact:!0});k.fixed.push(n);n=t.left+f[0].centre[0]-1;n=this.rendEng.createLine([n,b[0],n,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(n);g.push(g[0]+2*f[1].centre[0]);var u=f[1].clone().set({left:g[1],top:b[1]});k.modify.push(u);$.each(a.components,function(a,
c){if(c.hasOrnament("dot")){var g=e.bestDotPlacements(d,b,a);0<g.length&&k.modify.push(p.clone().set({left:u.left+2*f[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "cephalicus":g=[];c=this.rendEng.getGlyph("cephalicus");g.push(a.zone.ulx+c.centre[0]);t=c.clone().set({left:g[0],top:b[0]-c.centre[1]/2});k.modify.push(t);n=g[0]-c.centre[0]+1;n=this.rendEng.createLine([n,b[0],n,a.zone.lry],{strokeWidth:2,
interact:!0});k.fixed.push(n);n=t.left+c.centre[0]-2;n=this.rendEng.createLine([n,b[0],n,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(n);q=this.rendEng.getGlyph("liques_up");g.push(g[0]+c.centre[0]-q.centre[0]);u=q.clone().set({left:g[1],top:b[1]-q.centre[1]/2});k.modify.push(u);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var g=e.bestDotPlacements(d,b,a);0<g.length&&k.modify.push(p.clone().set({left:u.left+2*f[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+
b.pitchDiff}),g,2*f[0].centre[0],d);break;case "climacus.1":case "climacus.2":case "climacus.3":case "climacus.4":case "climacus.resupinus.1":case "climacus.resupinus.2":case "climacus.resupinus.3":case "climacus.resupinus.4":g=[];g.push(a.zone.ulx+f[0].centre[0]);q=f[0].centre[0];m=f[0].clone().set({left:g[0],top:b[0]});k.modify.push(m);n=m.left+f[0].centre[0]-1;n=this.rendEng.createLine([n,b[0],n,b[0]+1.5*d.delta_y],{strokeWidth:2,interact:!0});k.fixed.push(n);for(c=1;c<a.components.length;c++)g.push(g[c-
1]+2*f[c-1].centre[0]-1),1==c&&(g[1]+=q),n=f[c].clone().set({left:g[c],top:b[c]}),k.modify.push(n);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var h=e.bestDotPlacements(d,b,a);0<h.length&&k.modify.push(p.clone().set({left:g[a]+2*f[1].centre[0],top:h[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "torculus":g=[];g.push(a.zone.ulx+f[0].centre[0]);g.push(g[0]+2*f[0].centre[0]-1);g.push(g[1]+2*f[1].centre[0]-
1);t=f[0].clone().set({left:g[0],top:b[0]});k.modify.push(t);n=t.left+f[0].centre[0]-1;n=this.rendEng.createLine([n,b[0],n,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(n);u=f[1].clone().set({left:g[1],top:b[1]});k.modify.push(u);n=u.left+f[1].centre[0]-1;n=this.rendEng.createLine([n,b[1],n,b[2]],{strokeWidth:2,interact:!0});k.fixed.push(n);c=f[2].clone().set({left:g[2],top:b[2]});k.modify.push(c);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var h=e.bestDotPlacements(d,b,a);0<h.length&&
k.modify.push(p.clone().set({left:g[a]+2*f[1].centre[0],top:h[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "torculus.resupinus.1":case "torculus.resupinus.2":case "torculus.resupinus.3":case "torculus.resupinus.4":g=[];g.push(a.zone.ulx+f[0].centre[0]);t=f[0].clone().set({left:g[0],top:b[0]});k.modify.push(t);var c=this.rendEng.getGlyph("porrect_1"),r=c.clone().set({left:g[0]+f[0].centre[0]+c.centre[0]-1,top:b[1]+
c.centre[1]/2});k.modify.push(r);n=r.left-c.centre[0];m=b[0];l=r.top+c.centre[1];n=this.rendEng.createLine([n,b[1],n,m],{strokeWidth:2,interact:!0});k.fixed.push(n);"torculus.resupinus.1"==a.typeid?g.push(r.left+c.centre[0]-f[3].centre[0]):g.push(r.left+c.centre[0]+f[3].centre[0]);m=f[3].clone().set({left:g[1],top:b[3]});k.modify.push(m);n=r.left+c.centre[0]-1;n=this.rendEng.createLine([n,b[3],n,b[2]],{strokeWidth:2,interact:!0});k.fixed.push(n);"torculus.resupinus.2"==a.typeid&&(n=m.left+f[3].centre[0]-
...
d);break;case "podatus":g=[];q=0;1==Math.abs(a.components[1].pitchDiff)&&(q=1);c=this.rendEng.getGlyph("pes");g.push(a.zone.ulx+c.centre[0]);t=c.clone().set({left:g[0],top:b[0]-c.centre[1]/2+q});k.modify.push(t);n=t.left+c.centre[0]-1;n=this.rendEng.createLine([n,b[0],n,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(n);g.push(g[0]);u=f[1].clone().set({left:g[1],top:b[1]-q});k.modify.push(u);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var g=e.bestDotPlacements(d,b,a);0<g.length&&k.modify.push(p.clone().set({left:t.left+
2*f[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "podatus.subpunctis.1":case "podatus.subpunctis.2":case "podatus.subpunctis.3":case "podatus.subpunctis.4":case "podatus.subpunctis.resupinus.1":case "podatus.subpunctis.resupinus.2":case "podatus.subpunctis.resupinus.3":g=[];q=0;1==Math.abs(a.components[1].pitchDiff)&&(q=1);m=f[0].centre[0];c=this.rendEng.getGlyph("pes");g.push(a.zone.ulx+c.centre[0]);
t=c.clone().set({left:g[0],top:b[0]-c.centre[1]/2+q});k.modify.push(t);n=t.left+c.centre[0]-1;n=this.rendEng.createLine([n,b[0],n,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(n);g.push(g[0]);u=f[1].clone().set({left:g[1],top:b[1]-q});k.modify.push(u);for(c=2;c<a.components.length;c++)g.push(g[c-1]+2*f[c-1].centre[0]-1),2==c&&(g[c]+=m),n=f[c].clone().set({left:g[c],top:b[c]}),k.modify.push(n);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var h=e.bestDotPlacements(d,b,a);0<h.length&&
k.modify.push(p.clone().set({left:g[a]+2*f[a].centre[0],top:h[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "epiphonus":g=[];c=this.rendEng.getGlyph("podatus_ep");g.push(a.zone.ulx+c.centre[0]);t=c.clone().set({left:g[0],top:b[0]});k.modify.push(t);n=t.left+c.centre[0]-2;n=this.rendEng.createLine([n,b[0],n,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(n);q=this.rendEng.getGlyph("liques_down");g.push(g[0]+c.centre[0]-
q.centre[0]);u=q.clone().set({left:g[1],top:b[1]+q.centre[1]/2});k.modify.push(u);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var g=e.bestDotPlacements(d,b,a);0<g.length&&k.modify.push(p.clone().set({left:t.left+2*f[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "porrectus":n=a.components[0].pitchDiff+a.components[1].pitchDiff;c=null;switch(n){case -1:c=this.rendEng.getGlyph("porrect_1");
break;case -2:c=this.rendEng.getGlyph("porrect_2");break;case -3:c=this.rendEng.getGlyph("porrect_3");break;case -4:c=this.rendEng.getGlyph("porrect_4");break;default:c=this.rendEng.getGlyph("porrect_4")}r=c.clone().set({left:a.zone.ulx+c.centre[0],top:b[0]+c.centre[1]/2});k.modify.push(r);n=r.left-c.centre[0]+1;m=a.zone.lry;l=r.top+c.centre[1];a.zone.lry<r.top+l&&(m=l);n=this.rendEng.createLine([n,b[0],n,m],{strokeWidth:2,interact:!0});k.fixed.push(n);g=r.left+c.centre[0]-f[2].centre[0];m=f[2].clone().set({left:g,
top:b[2]});n=m.left+f[2].centre[0]-1;n=this.rendEng.createLine([n,b[2],n,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(n);k.modify.push(m);a.components[2].hasOrnament("dot")&&(l=this.bestDotPlacements(d,b,2),0<l.length&&k.modify.push(p.clone().set({left:m.left+2*f[2].centre[0],top:l[0]})));this.drawLedgerLines([a.rootSystemPos+a.components[2].pitchDiff],[g],2*f[2].centre[0],d);break;case "porrectus.flexus":c=null;n=a.components[0].pitchDiff+a.components[1].pitchDiff;switch(n){case -1:c=this.rendEng.getGlyph("porrect_1");
break;case -2:c=this.rendEng.getGlyph("porrect_2");break;case -3:c=this.rendEng.getGlyph("porrect_3");break;case -4:c=this.rendEng.getGlyph("porrect_4");break;default:c=this.rendEng.getGlyph("porrect_4")}r=c.clone().set({left:a.zone.ulx+c.centre[0],top:b[0]+c.centre[1]/2});k.modify.push(r);n=r.left-c.centre[0]+1;m=a.zone.lry;l=r.top+c.centre[1];a.zone.lry<r.top+l&&(m=l);n=this.rendEng.createLine([n,b[0],n,m],{strokeWidth:2,interact:!0});k.fixed.push(n);g=[];g.push(r.left+c.centre[0]+f[2].centre[0]);
m=f[2].clone().set({left:g[0],top:b[2]});n=g[0]-f[2].centre[0]-1;n=this.rendEng.createLine([n,b[2],n,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(n);k.modify.push(m);for(c=3;c<a.components.length;c++)g.push(g[g.length-1]+2*f[c].centre[0]-1),n=f[c].clone().set({left:g[g.length-1],top:b[c]}),k.modify.push(n);for(c=2;c<a.components.length;c++)n=a.components[c],n.hasOrnament("dot")&&(l=e.bestDotPlacements(d,b,q),0<l.length&&k.modify.push(p.clone().set({left:t.left+2*f[1].centre[0],top:l[0]})));this.drawLedgerLines($.map(a.components,
...
default:c=this.rendEng.getGlyph("porrect_4")}r=c.clone().set({left:a.zone.ulx+c.centre[0],top:b[0]+c.centre[1]/2});k.modify.push(r);n=r.left-c.centre[0]+1;m=a.zone.lry;l=r.top+c.centre[1];a.zone.lry<r.top+l&&(m=l);n=this.rendEng.createLine([n,b[0],n,m],{strokeWidth:2,interact:!0});k.fixed.push(n);g=[];g.push(r.left+c.centre[0]-f[2].centre[0]);m=f[2].clone().set({left:g[0],top:b[2]});n=g[0]+f[2].centre[0]-1;n=this.rendEng.createLine([n,b[2],n,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(n);k.modify.push(m);
g.push(r.left+c.centre[0]+2*f[2].centre[0]);for(c=3;c<a.components.length;c++)n=f[c].clone().set({left:g[c-2],top:b[c]}),k.modify.push(n),c+1<a.components.length&&g.push(g[c-2]+2*f[c].centre[0]-1);for(c=2;c<a.components.length;c++)n=a.components[c],n.hasOrnament("dot")&&(l=e.bestDotPlacements(d,b,c),0<l.length&&k.modify.push(p.clone().set({left:g[c-2]+2*f[c].centre[0],top:l[0]})));this.drawLedgerLines($.map(a.components,function(b,c){if(2<=c)return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],
d);break;case "compound.1":c=null;n=a.components[0].pitchDiff+a.components[1].pitchDiff;switch(n){case -1:c=this.rendEng.getGlyph("porrect_1");break;case -2:c=this.rendEng.getGlyph("porrect_2");break;case -3:c=this.rendEng.getGlyph("porrect_3");break;case -4:c=this.rendEng.getGlyph("porrect_4");break;default:c=this.rendEng.getGlyph("porrect_4")}r=c.clone().set({left:a.zone.ulx+c.centre[0],top:b[0]+c.centre[1]/2});k.modify.push(r);n=r.left-c.centre[0]+1;m=a.zone.lry;l=r.top+c.centre[1];a.zone.lry<
r.top+l&&(m=l);n=this.rendEng.createLine([n,b[0],n,m],{strokeWidth:2,interact:!0});k.fixed.push(n);g=[];g.push(r.left+c.centre[0]+f[2].centre[0]);for(c=2;c<a.components.length;c++)n=f[c].clone().set({left:g[c-2],top:b[c]}),k.modify.push(n),n=g[c-2]-f[2].centre[0]-1,n=this.rendEng.createLine([n,b[c],n,b[c-1]],{strokeWidth:2,interact:!0}),k.fixed.push(n),c+1<a.components.length&&g.push(g[c-2]+2*f[c].centre[0]-1);for(c=2;c<a.components.length;c++)n=a.components[c],n.hasOrnament("dot")&&(l=e.bestDotPlacements(d,
b,c),0<l.length&&k.modify.push(p.clone().set({left:g[c-2]+2*f[c].centre[0],top:l[0]})));this.drawLedgerLines($.map(a.components,function(b,c){if(2<=c)return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "compound.2":c=null;n=a.components[0].pitchDiff+a.components[1].pitchDiff;switch(n){case -1:c=this.rendEng.getGlyph("porrect_1");break;case -2:c=this.rendEng.getGlyph("porrect_2");break;case -3:c=this.rendEng.getGlyph("porrect_3");break;case -4:c=this.rendEng.getGlyph("porrect_4");
break;default:c=this.rendEng.getGlyph("porrect_4")}r=c.clone().set({left:a.zone.ulx+c.centre[0],top:b[0]+c.centre[1]/2});k.modify.push(r);n=r.left-c.centre[0]+1;m=a.zone.lry;l=r.top+c.centre[1];a.zone.lry<r.top+l&&(m=l);n=this.rendEng.createLine([n,b[0],n,m],{strokeWidth:2,interact:!0});k.fixed.push(n);g=[];g.push(r.left+c.centre[0]+f[2].centre[0]);m=this.rendEng.getGlyph("pes");q=0;1==Math.abs(a.components[3].pitchDiff-a.components[2].pitchDiff)&&(q=1);n=g[0]-f[2].centre[0];n=this.rendEng.createLine([n,
b[1],n,b[2]],{strokeWidth:2,interact:!0});k.fixed.push(n);l=m.clone().set({left:g[0],top:b[2]-m.centre[1]/2+q});k.modify.push(l);l=g[0]+m.centre[0]-1;l=this.rendEng.createLine([l,b[2],l,b[3]],{strokeWidth:2,interact:!0});k.fixed.push(l);g.push(g[0]);u=f[3].clone().set({left:g[1],top:b[3]-q});k.modify.push(u);for(c=2;c<a.components.length;c++)n=a.components[c],n.hasOrnament("dot")&&(l=e.bestDotPlacements(d,b,c),0<l.length&&k.modify.push(p.clone().set({left:g[c-2]+2*f[c].centre[0],top:l[0]})));this.drawLedgerLines($.map(a.components,
function(b,c){if(2<=c)return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "scandicus.1":case "scandicus.2":case "scandicus.3":case "scandicus.4":n=a.components.length;m=this.rendEng.getGlyph("pes");r=a.zone.ulx-f[0].centre[0];g=[];for(c=0;c<n-1;c+=2)for(q=0,1==Math.abs(a.components[c+1].pitchDiff-a.components[c].pitchDiff)&&(q=1),r+=2*m.centre[0]-1,g.push(r),l=m.clone().set({left:r,top:b[c]-m.centre[1]/2+q}),k.modify.push(l),l=r+m.centre[0]-1,l=this.rendEng.createLine([l,b[c],l,
b[c+1]],{strokeWidth:2,interact:!0}),k.fixed.push(l),g.push(r),u=f[c+1].clone().set({left:r,top:b[c+1]-q}),k.modify.push(u),h=c;h<c+2;h++)a.components[h].hasOrnament("dot")&&(l=this.bestDotPlacements(d,b,h),0<l.length&&k.modify.push(p.clone().set({left:u.left+2*f[h].centre[0],top:l[0]})));1==a.components.length%2&&(r+=2*f[n-1].centre[0]-1,g.push(r),c=f[n-1].clone().set({left:r,top:b[n-1]}),k.modify.push(c),a.components[n-1].hasOrnament("dot")&&(l=this.bestDotPlacements(d,b,n-1),0<l.length&&k.modify.push(p.clone().set({left:c.left+
2*f[n-1].centre[0],top:l[0]}))),c=r+f[n-1].centre[0]-2,c=this.rendEng.createLine([c,b[n-1],c,a.zone.lry-(a.zone.lry-a.zone.uly)/2],{strokeWidth:2,interact:!0}),k.fixed.push(c));this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "scandicus.flexus.1":m=this.rendEng.getGlyph("pes");g=[];g.push(a.zone.ulx+m.centre[0]);q=0;1==Math.abs(a.components[1].pitchDiff-a.components[0].pitchDiff)&&(q=1);l=m.clone().set({left:g[0],top:b[0]-m.centre[1]/
2+q});k.modify.push(l);l=g[0]+m.centre[0]-1;l=this.rendEng.createLine([l,b[0],l,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(l);g.push(g[0]);u=f[1].clone().set({left:g[1],top:b[1]-q});k.modify.push(u);g.push(g[1]+3*f[2].centre[0]);c=f[2].clone().set({left:g[2],top:b[2]});k.modify.push(c);n=g[2]-f[2].centre[0]+1;n=this.rendEng.createLine([n,b[2],n,b[2]+1.5*d.delta_y],{strokeWidth:2,interact:!0});k.fixed.push(n);n=g[2]+f[2].centre[0]-1;n=this.rendEng.createLine([n,b[2],n,b[3]],{strokeWidth:2,interact:!0});
k.fixed.push(n);g.push(g[2]+2*f[3].centre[0]);c=f[3].clone().set({left:g[3],top:b[3]});k.modify.push(c);for(h=2;h<a.components.length;h++)a.components[h].hasOrnament("dot")&&(l=e.bestDotPlacements(d,b,h),0<l.length&&k.modify.push(p.clone().set({left:g[h]+2*f[h].centre[0],top:l[0]})));this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "scandicus.flexus.2":m=this.rendEng.getGlyph("pes");g=[];g.push(a.zone.ulx+m.centre[0]);q=0;1==
Math.abs(a.components[1].pitchDiff-a.components[0].pitchDiff)&&(q=1);l=m.clone().set({left:g[0],top:b[0]-m.centre[1]/2+q});k.modify.push(l);l=g[0]+m.centre[0]-1;l=this.rendEng.createLine([l,b[0],l,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(l);g.push(g[0]);u=f[1].clone().set({left:g[1],top:b[1]-q});k.modify.push(u);for(c=2;c<a.components.length;c++)g.push(g[c-1]+2*f[c-1].centre[0]-1),m=f[c].clone().set({left:g[c],top:b[c]}),k.modify.push(m),c+1<a.components.length&&(n=g[c]+f[c].centre[0]-1,n=
this.rendEng.createLine([n,b[c],n,b[c+1]],{strokeWidth:2,interact:!0}),k.fixed.push(n));for(q=0;q<a.components.length;q++)a.components[q].hasOrnament("dot")&&(l=e.bestDotPlacements(d,b,q),0<l.length&&k.modify.push(p.clone().set({left:g[q]+2*f[1].centre[0],top:l[0]})));this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "scandicus.flexus.3":g=[];g.push(a.zone.ulx+f[0].centre[0]);t=f[0].clone().set({left:g[0],top:b[0]});k.modify.push(t);
q=0;1==Math.abs(a.components[2].pitchDiff-a.components[1].pitchDiff)&&(q=1);m=this.rendEng.getGlyph("pes");g.push(g[0]+2*f[0].centre[0]-1);l=m.clone().set({left:g[1],top:b[1]-m.centre[1]/2+q});k.modify.push(l);l=g[1]+m.centre[0]-1;l=this.rendEng.createLine([l,b[1],l,b[2]],{strokeWidth:2,interact:!0});k.fixed.push(l);g.push(g[1]);c=f[2].clone().set({left:g[2],top:b[2]-q});k.modify.push(c);for(c=3;c<a.components.length;c++)g.push(g[c-1]+2*f[c-1].centre[0]-1),m=f[c].clone().set({left:g[c],top:b[c]}),
k.modify.push(m),c+1<a.components.length&&(n=g[c]+f[c].centre[0]-1,n=this.rendEng.createLine([n,b[c],n,b[c+1]],{strokeWidth:2,interact:!0}),k.fixed.push(n));for(q=0;q<a.components.length;q++)a.components[q].hasOrnament("dot")&&(l=e.bestDotPlacements(d,b,q),0<l.length&&k.modify.push(p.clone().set({left:g[q]+2*f[q].centre[0],top:l[0]})));this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "scandicus.subpunctis.1":case "scandicus.subpunctis.2":m=
this.rendEng.getGlyph("pes");g=[];g.push(a.zone.ulx+m.centre[0]);q=0;1==Math.abs(a.components[1].pitchDiff-a.components[0].pitchDiff)&&(q=1);l=m.clone().set({left:g[0],top:b[0]-m.centre[1]/2+q});k.modify.push(l);l=g[0]+m.centre[0]-1;l=this.rendEng.createLine([l,b[0],l,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(l);g.push(g[0]);u=f[1].clone().set({left:g[1],top:b[1]-q});k.modify.push(u);g.push(g[1]+2*f[2].centre[0]);c=f[2].clone().set({left:g[2],top:b[2]});k.modify.push(c);n=g[2]+f[2].centre[0]-
1;n=this.rendEng.createLine([n,b[2],n,b[2]+1.5*d.delta_y],{strokeWidth:2,interact:!0});k.fixed.push(n);q=f[2].centre[0];for(c=3;c<a.components.length;c++)g.push(g[c-1]+2*f[c-1].centre[0]-1),3==c&&(g[c]+=q),n=f[c].clone().set({left:g[c],top:b[c]}),k.modify.push(n);for(q=0;q<a.components.length;q++)a.components[q].hasOrnament("dot")&&(l=e.bestDotPlacements(d,b,q),0<l.length&&k.modify.push(p.clone().set({left:g[q]+2*f[q].centre[0],top:l[0]})));this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+
...
b.pitchDiff}),g,2*f[0].centre[0],d);break;case "virga":m=a.zone.ulx+f[0].centre[0];c=f[0].clone().set({left:m,top:b[0]});k.modify.push(c);a.components[0].hasOrnament("dot")&&(l=this.bestDotPlacements(d,b,0),k.modify.push(p.clone().set({left:c.left+2*f[0].centre[0],top:l[0]})));l=c.left+f[0].centre[0]-1;h=this.rendEng.createLine([l,b[0],l,b[0]+1.5*d.delta_y],{strokeWidth:2,interact:!0});k.fixed.push(h);this.drawLedgerLines([a.rootSystemPos],[m],2*f[0].centre[0],d);break;case "bivirga":case "trivirga":g=
...
b.pitchDiff}),g,2*f[0].centre[0],d);break;case "clivis":g=[];g.push(a.zone.ulx+f[0].centre[0]);var q=f[0].clone().set({left:g[0],top:b[0]});k.modify.push(q);l=q.left+f[0].centre[0]-1;h=this.rendEng.createLine([l,b[0],l,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(h);g.push(g[0]+2*f[1].centre[0]);var n=f[1].clone().set({left:g[1],top:b[1]});k.modify.push(n);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var g=e.bestDotPlacements(d,b,a);0<g.length&&k.modify.push(p.clone().set({left:n.left+
2*f[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "cephalicus":g=[];m=this.rendEng.getGlyph("cephalicus");g.push(a.zone.ulx+m.centre[0]);q=m.clone().set({left:g[0],top:b[0]-m.centre[1]/2});k.modify.push(q);c=g[0]-m.centre[0]+1;h=this.rendEng.createLine([c,b[0],c,a.zone.lry],{strokeWidth:2,interact:!0});k.fixed.push(h);l=q.left+m.centre[0]-2;h=this.rendEng.createLine([l,b[0],l,b[1]],{strokeWidth:2,
interact:!0});k.fixed.push(h);c=this.rendEng.getGlyph("liques_up");g.push(g[0]+m.centre[0]-c.centre[0]);n=c.clone().set({left:g[1],top:b[1]-c.centre[1]/2});k.modify.push(n);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var g=e.bestDotPlacements(d,b,a);0<g.length&&k.modify.push(p.clone().set({left:n.left+2*f[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "climacus.1":case "climacus.2":case "climacus.3":case "climacus.4":g=
[];g.push(a.zone.ulx+f[0].centre[0]);m=f[0].centre[0];c=f[0].clone().set({left:g[0],top:b[0]});k.modify.push(c);l=c.left+f[0].centre[0]-1;h=this.rendEng.createLine([l,b[0],l,b[0]+1.5*d.delta_y],{strokeWidth:2,interact:!0});k.fixed.push(h);for(c=1;c<a.components.length;c++)g.push(g[c-1]+2*f[c-1].centre[0]-1),1==c&&(g[1]+=m),l=f[c].clone().set({left:g[c],top:b[c]}),k.modify.push(l);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var h=e.bestDotPlacements(d,b,a);0<h.length&&k.modify.push(p.clone().set({left:g[a]+
2*f[1].centre[0],top:h[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "torculus":g=[];g.push(a.zone.ulx+f[0].centre[0]);g.push(g[0]+2*f[0].centre[0]-1);g.push(g[1]+2*f[1].centre[0]-1);q=f[0].clone().set({left:g[0],top:b[0]});k.modify.push(q);l=q.left+f[0].centre[0]-1;h=this.rendEng.createLine([l,b[0],l,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(h);n=f[1].clone().set({left:g[1],top:b[1]});k.modify.push(n);l=n.left+
...
[];g.push(a.zone.ulx+f[0].centre[0]);q=f[0].clone().set({left:g[0],top:b[0]});k.modify.push(q);var l=this.rendEng.getGlyph("porrect_1"),t=l.clone().set({left:g[0]+f[0].centre[0]+l.centre[0]-1,top:b[1]+l.centre[1]/2});k.modify.push(t);var c=t.left-l.centre[0],h=b[0],u=t.top+l.centre[1],h=this.rendEng.createLine([c,b[1],c,h],{strokeWidth:2,interact:!0});k.fixed.push(h);"torculus.resupinus.1"==a.typeid?g.push(t.left+l.centre[0]-f[3].centre[0]):g.push(t.left+l.centre[0]+f[3].centre[0]);c=f[3].clone().set({left:g[1],
top:b[3]});k.modify.push(c);l=t.left+l.centre[0]-1;h=this.rendEng.createLine([l,b[3],l,b[2]],{strokeWidth:2,interact:!0});k.fixed.push(h);"torculus.resupinus.2"==a.typeid&&(l=c.left+f[3].centre[0]-1,h=this.rendEng.createLine([l,b[4],l,b[3]],{strokeWidth:2,interact:!0}),k.fixed.push(h));for(c=4;c<a.components.length;c++)g.push(g[g.length-1]+2*f[c].centre[0]-1),l=f[c].clone().set({left:g[g.length-1],top:b[c]}),k.modify.push(l);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var h=e.bestDotPlacements(d,
b,a);0<h.length&&k.modify.push(p.clone().set({left:g[a]+2*f[1].centre[0],top:h[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "podatus":g=[];g.push(a.zone.ulx+f[0].centre[0]);q=f[0].clone().set({left:g[0],top:b[0]});k.modify.push(q);g.push(g[0]+2*f[1].centre[0]);n=f[1].clone().set({left:g[1],top:b[1]});k.modify.push(n);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var g=e.bestDotPlacements(d,b,a);0<g.length&&
k.modify.push(p.clone().set({left:q.left+2*f[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "epiphonus":g=[];m=this.rendEng.getGlyph("podatus_ep");g.push(a.zone.ulx+m.centre[0]);q=m.clone().set({left:g[0],top:b[0]});k.modify.push(q);l=q.left+m.centre[0]-2;h=this.rendEng.createLine([l,b[0],l,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(h);c=this.rendEng.getGlyph("liques_down");g.push(g[0]+m.centre[0]-
c.centre[0]);n=c.clone().set({left:g[1],top:b[1]+c.centre[1]/2});k.modify.push(n);$.each(a.components,function(a,c){if(c.hasOrnament("dot")){var g=e.bestDotPlacements(d,b,a);0<g.length&&k.modify.push(p.clone().set({left:q.left+2*f[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "porrectus":m=a.components[0].pitchDiff+a.components[1].pitchDiff;l=null;switch(m){case -1:l=this.rendEng.getGlyph("porrect_1");
break;case -2:l=this.rendEng.getGlyph("porrect_2");break;case -3:l=this.rendEng.getGlyph("porrect_3");break;case -4:l=this.rendEng.getGlyph("porrect_4");break;default:l=this.rendEng.getGlyph("porrect_4")}t=l.clone().set({left:a.zone.ulx+l.centre[0],top:b[0]+l.centre[1]/2});k.modify.push(t);c=t.left-l.centre[0]+1;h=a.zone.lry;u=t.top+l.centre[1];a.zone.lry<t.top+u&&(h=u);h=this.rendEng.createLine([c,b[0],c,h],{strokeWidth:2,interact:!0});k.fixed.push(h);g=t.left+l.centre[0]-f[2].centre[0];c=f[2].clone().set({left:g,
top:b[2]});l=c.left+f[2].centre[0]-1;h=this.rendEng.createLine([l,b[2],l,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(h);k.modify.push(c);a.components[2].hasOrnament("dot")&&(l=this.bestDotPlacements(d,b,2),0<l.length&&k.modify.push(p.clone().set({left:c.left+2*f[2].centre[0],top:l[0]})));this.drawLedgerLines([a.rootSystemPos+a.components[2].pitchDiff],[g],2*f[2].centre[0],d);break;case "porrectus.flexus":l=this.rendEng.getGlyph("porrect_1");t=l.clone().set({left:a.zone.ulx+l.centre[0],top:b[0]+
l.centre[1]/2});k.modify.push(t);c=t.left-l.centre[0]+1;h=a.zone.lry;u=t.top+l.centre[1];a.zone.lry<t.top+u&&(h=u);h=this.rendEng.createLine([c,b[0],c,h],{strokeWidth:2,interact:!0});k.fixed.push(h);g=[];g.push(t.left+l.centre[0]+f[2].centre[0]);c=f[2].clone().set({left:g[0],top:b[2]});l=g[0]-f[2].centre[0]-1;h=this.rendEng.createLine([l,b[2],l,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(h);k.modify.push(c);for(c=3;c<a.components.length;c++)g.push(g[g.length-1]+2*f[c].centre[0]-1),l=f[c].clone().set({left:g[g.length-
1],top:b[c]}),k.modify.push(l);for(c=2;c<a.components.length;c++)a.components[c].hasOrnament("dot")&&(l=e.bestDotPlacements(d,b,m),0<l.length&&k.modify.push(p.clone().set({left:q.left+2*f[1].centre[0],top:l[0]})));break;case "scandicus.1":case "scandicus.2":case "scandicus.3":case "scandicus.4":m=a.components.length;t=a.zone.ulx-f[0].centre[0];g=[];for(c=0;c<m-1;c+=2)for(yoffset=0,1==Math.abs(a.components[c+1].pitchDiff-a.components[c].pitchDiff)&&(yoffset=1),t+=2*f[c].centre[0]-1,g.push(t),l=f[c].clone().set({left:g[c],
top:b[c]-f[c].centre[1]/2+yoffset}),k.modify.push(l),g.push(g[c]+2*f[c+1].centre[0]),n=f[c+1].clone().set({left:g[c+1],top:b[c+1]-yoffset}),k.modify.push(n),h=c;h<c+2;h++)a.components[h].hasOrnament("dot")&&(l=this.bestDotPlacements(d,b,h),0<l.length&&k.modify.push(p.clone().set({left:n.left+2*f[h].centre[0],top:l[0]})));1==a.components.length%2&&(t+=2*f[m-1].centre[0]-1,g.push(t),c=f[m-1].clone().set({left:t,top:b[m-1]}),k.modify.push(c),a.components[m-1].hasOrnament("dot")&&(l=this.bestDotPlacements(d,
b,m-1),0<l.length&&k.modify.push(p.clone().set({left:c.left+2*f[m-1].centre[0],top:l[0]}))));this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d);break;case "scandicus.flexus.1":g=[];g.push(a.zone.ulx+f[0].centre[0]);q=f[0].clone().set({left:g[0],top:b[0]});k.modify.push(q);l=q.left+f[0].centre[0];h=this.rendEng.createLine([l,b[0],l,b[1]],{strokeWidth:2,interact:!0});k.fixed.push(h);g.push(g[0]+2*f[1].centre[0]);n=f[1].clone().set({left:g[1],
top:b[1]});k.modify.push(n);for(h=0;2>h;h++)a.components[h].hasOrnament("dot")&&(l=this.bestDotPlacements(d,b,h),0<l.length&&k.modify.push(p.clone().set({left:g[h]+2*f[h].centre[0],top:l[0]})));g.push(g[1]+2*f[2].centre[0]);c=f[2].clone().set({left:g[2],top:b[2]});k.modify.push(c);l=g[2]+f[2].centre[0]-1;h=this.rendEng.createLine([l,b[2],l,b[3]],{strokeWidth:2,interact:!0});k.fixed.push(h);g.push(g[2]+2*f[3].centre[0]);c=f[3].clone().set({left:g[3],top:b[3]});k.modify.push(c);for(h=2;h<a.components.length;h++)a.components[h].hasOrnament("dot")&&
(l=e.bestDotPlacements(d,b,m),0<l.length&&k.modify.push(p.clone().set({left:n.left+2*f[1].centre[0],top:l[0]})));this.drawLedgerLines($.map(a.components,function(b){return a.rootSystemPos+b.pitchDiff}),g,2*f[0].centre[0],d)}this.drawing=this.rendEng.draw(k,{group:!0,selectable:a.props.interact,eleRef:a})[0];$(a).trigger("mUpdateBoundingBox",this.drawing)};Toe.View.SquareNoteInteraction=function(a,b,d,e){Toe.View.Interaction.call(this,a,b,d,e);b={initMode:"edit"};$.extend(b,e);this.systemDwg=this.clefDwg=this.divisionDwg=this.punctDwg=null;e=a.getGlyph("punctum").clone();this.punctWidth=e.width*a.getGlobalScale();this.punctHeight=e.height*a.getGlobalScale();this.objMoving=!1;$("#btn_edit").bind("click.edit",{gui:this,parentDivId:"#gui-sidebar"},this.handleEdit);$("#btn_insert").bind("click.insert",{gui:this,parentDivId:"#gui-sidebar"},this.handleInsert);
$("#btn_"+b.initMode).trigger("click");this.bindHotKeys()};Toe.View.SquareNoteInteraction.prototype=new Toe.View.Interaction;Toe.View.SquareNoteInteraction.prototype.constructor=Toe.View.SquareNoteInteraction;
...
if(h){var c=[];h.eleRef?c.push(h):$.each(h.objects,function(a,b){c.push(b)});$.each(c,function(a,d){var m=d.eleRef;if(m instanceof Toe.Model.Clef){var l=d.left,g=d.top;1<c.length&&(l=h.left+d.left,g=h.top+d.top);l=m.system.getSystemSnapCoordinates({x:l,y:g},null,{ignoreEle:m});l=-Math.round((l.y-m.system.zone.uly)/(m.system.delta_y/2));m.setSystemPosition(l);var q=m.system.getPitchedElements({neumes:!0,custos:!1});0<q.length&&m.system.getActingClefByEle(q[0])==m&&(l=b.page.getPreviousSystem(m.system))&&
b.handleUpdatePrevCustos(q[0].components[0].pname,q[0].components[0].oct,l);var l=$.map(m.system.getPitchedElements({clef:m}),function(a){if(a instanceof Toe.Model.Neume){var c=[];$.each(a.components,function(a,b){c.push({pname:b.pname,oct:b.oct})});return{id:a.id,noteInfo:c}}if(a instanceof Toe.Model.Custos){var d=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);$.post(b.apiprefix+"/move/custos",{id:a.id,ulx:d[0],uly:d[1],lrx:d[2],lry:d[3]}).error(function(){this.showAlert("Server failed to move custos. Client and server are not synchronized.")})}}),
g=m.system.props.numLines+m.props.systemPos/2,q=b.getOutputBoundingBox([m.zone.ulx,m.zone.uly,m.zone.lrx,m.zone.lry]),n={id:m.id,line:g,ulx:q[0],uly:q[1],lrx:q[2],lry:q[3],pitchInfo:l};$.post(b.apiprefix+"/move/clef",{data:JSON.stringify(n)}).error(function(){this.showAlert("Server failed to move clef. Client and server are not synchronized.")})}else if(m instanceof Toe.Model.Neume){l=d.left;g=d.top;1<c.length&&(l=h.left+d.left,g=h.top+d.top);var t={x:l,y:m.system.zone.uly-m.rootSystemPos*m.system.delta_y/
2-f},u=b.page.getClosestSystem(t),r=u.getSystemSnapCoordinates(t,d.currentWidth,{ignoreEle:m}),l=Math.round((u.zone.uly-r.y)/(u.delta_y/2)),q=r.x-d.currentWidth/2,g=g-d.currentHeight/2-(t.y-r.y),q=[q,g,q+d.currentWidth,g+d.currentHeight];m.setBoundingBox(q);t=m.rootSystemPos;$.each(m.components,function(a,b){var c=u.calcPitchFromCoords({x:r.x,y:r.y-u.delta_y/2*b.pitchDiff});b.setPitchInfo(c.pname,c.oct)});$(m).trigger("vEraseDrawing");m.system.removeElementByRef(m);g=u.addNeume(m);1==c.length&&$(m).trigger("vSelectDrawing");
q=b.getOutputBoundingBox([m.zone.ulx,m.zone.uly,m.zone.lrx,m.zone.lry]);n={id:m.id,ulx:q[0],uly:q[1],lrx:q[2],lry:q[3]};t!=l?(n.pitchInfo=[],$.each(m.components,function(a,b){n.pitchInfo.push({pname:b.pname,oct:b.oct})}),m==u.elements[1]&&(l=b.page.getPreviousSystem(u))&&b.handleUpdatePrevCustos(m.components[0].pname,m.components[0].oct,l)):n.pitchInfo=null;g+1<u.elements.length?n.beforeid=u.elements[g+1].id:(l=b.page.getNextSystem(u),n.beforeid=l.id);$.post(b.apiprefix+"/move/neume",{data:JSON.stringify(n)}).error(function(){this.showAlert("Server failed to move neume. Client and server are not synchronized.")})}else if(m instanceof
Toe.Model.Division){l=d.left;g=d.top;1<c.length&&(l+=h.left,g+=h.top);t={x:l,y:g};l=b.page.getClosestSystem(t);r=l.getSystemSnapCoordinates(t,d.currentWidth,{x:!0,y:!1});switch(m.type){case Toe.Model.Division.Type.div_small:r.y=l.zone.uly;break;case Toe.Model.Division.Type.div_minor:r.y=l.zone.uly+(l.zone.lry-l.zone.uly)/2;break;case Toe.Model.Division.Type.div_major:r.y=l.zone.uly+(l.zone.lry-l.zone.uly)/2;break;case Toe.Model.Division.Type.div_final:r.y=l.zone.uly+(l.zone.lry-l.zone.uly)/2}m.system.removeElementByRef(m);
b.rendEng.canvas.remove(d);b.rendEng.repaint();q=r.x-d.currentWidth/2;g=r.y-d.currentHeight/2;q=[q,g,q+d.currentWidth,g+d.currentHeight];m.setBoundingBox(q);q=l.addDivision(m);1==c.length&&m.selectDrawing();g=null;q+1<l.elements.length?g=l.elements[q+1].id:(l=b.page.getNextSystem(l),g=l.id);q=b.getOutputBoundingBox([m.zone.ulx,m.zone.uly,m.zone.lrx,m.zone.lry]);$.post(b.apiprefix+"/move/division",{id:m.id,ulx:q[0],uly:q[1],lrx:q[2],lry:q[3],beforeid:g}).error(function(){this.showAlert("Server failed to move division. Client and server are not synchronized.")})}else m instanceof
Toe.Model.Custos&&(l=d.left,g=d.top,1<c.length&&(d.left=l+e,d.top=g+f))});1<c.length&&b.rendEng.canvas.discardActiveGroup();b.rendEng.repaint()}b.objMoving=!1}});$("#btn_delete").unbind("click");$("#btn_neumify").unbind("click");$("#btn_ungroup").unbind("click");$("#btn_delete").bind("click.edit",{gui:b},b.handleDelete);$("#btn_neumify").bind("click.edit",{gui:b},b.handleNeumify);$("#btn_neumify_liquescence").bind("click.edit",{gui:b,modifier:"alt"},b.handleNeumify);$("#btn_ungroup").bind("click.edit",
{gui:b},b.handleUngroup)};
Toe.View.SquareNoteInteraction.prototype.handleDotToggle=function(a){var b=a.data.gui,d=a.data.punctum;if(a=d.components[0].hasOrnament("dot"))d.components[0].removeOrnament("dot");else{var e=new Toe.Model.Ornament("dot",{form:"aug"});d.components[0].addOrnament(e)}d.syncDrawing();e=b.getOutputBoundingBox([d.zone.ulx,d.zone.uly,d.zone.lrx,d.zone.lry]);d={id:d.id,dotform:"aug",ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]};a?$.post(b.apiprefix+"/delete/dot",d).error(function(){this.showAlert("Server failed to remove dot from the punctum. Client and server are not synchronized.")}):$.post(b.apiprefix+
"/insert/dot",d).error(function(){this.showAlert("Server failed to add a dot to the punctum. Client and server are not synchronized.")});$(this).toggleClass("active")};
Toe.View.SquareNoteInteraction.prototype.handleHeadShapeChange=function(a){var b=a.data.gui,d=a.data.shape;a=a.data.punctum;a.components[0].setHeadShape(d);"virga"==d?(a.name="Virga",a.typeid="virga"):"cavum"==d&&(a.name="Cavum",a.typeid="cavum");a.syncDrawing();var e=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);$.post(b.apiprefix+"/update/neume/headshape",{id:a.id,shape:d,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]}).error(function(){this.showAlert("Server failed to change note head shape. Client and server are not synchronized.")})};
...
b){c.push({pname:b.pname,oct:b.oct})});return{id:a.id,noteInfo:c}}if(a instanceof Toe.Model.Custos){var d=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);$.post(b.apiprefix+"/move/custos",{id:a.id,ulx:d[0],uly:d[1],lrx:d[2],lry:d[3]}).error(function(){this.showAlert("Server failed to move custos. Client and server are not synchronized.")})}});f=b.getOutputBoundingBox([d.zone.ulx,d.zone.uly,d.zone.lrx,d.zone.lry]);$.post(b.apiprefix+"/update/clef/shape",{data:JSON.stringify({id:d.id,
shape:a,ulx:f[0],uly:f[1],lrx:f[2],lry:f[3],pitchInfo:e})}).error(function(){this.showAlert("Server failed to update clef shape. Client and server are not synchronized.")});$(this).toggleClass("active")}};
Toe.View.SquareNoteInteraction.prototype.handleNeumify=function(a){var b=a.data.gui;a=a.data.modifier;var d=b.rendEng.canvas.getActiveGroup();if(d){var e=[],f=null;$.each(d.getObjects(),function(a,b){b.eleRef instanceof Toe.Model.Neume&&(f||(f=b.eleRef.system),b.eleRef.system==f&&e.push(b))});if(!(2>e.length)){e.sort(function(a,b){return a.eleRef.zone.ulx-b.eleRef.zone.ulx});var h=new Toe.Model.SquareNoteNeume({modifier:a});numPunct=0;var c=[],p=Number.MAX_VALUE,k=Number.MAX_VALUE,m=Number.MIN_VALUE;
$.each(e,function(a,g){$.merge(h.components,g.eleRef.components);numPunct+=g.eleRef.components.length;c.push(g.eleRef.id);var e=d.top+g.top;p=Math.min(p,d.left+g.left-g.currentHeight/2);k=Math.min(k,e-g.currentHeight/2);m=Math.max(m,e+g.currentHeight/2);f.removeElementByRef(g.eleRef);b.rendEng.canvas.remove(g)});h.setBoundingBox([p,k,p+numPunct*b.punctWidth,m]);a=new Toe.View.NeumeView(b.rendEng,b.page.documentType);new Toe.Ctrl.NeumeController(h,a);f.addNeume(h);a=b.getOutputBoundingBox([h.zone.ulx,
h.zone.uly,h.zone.lrx,h.zone.lry]);var l=h.typeid,g=$.map(h.components,function(a){return a.props.type});a=JSON.stringify({nids:c.join(","),typeid:l,headShapes:g,ulx:a[0],uly:a[1],lrx:a[2],lry:a[3]});$.post(b.apiprefix+"/neumify",{data:a},function(a){h.id=JSON.parse(a).id}).error(function(){this.showAlert("Server failed to neumify selected neumes. Client and server are not synchronized.")});b.rendEng.canvas.discardActiveGroup();$(h).trigger("vSelectDrawing");b.rendEng.repaint()}}};
...
!1,e=function(a){var e={modify:[],fixed:[]},c=null,p=b.rendEng.getGlyph("punctum");a?c={left:-50,top:-50}:(c={left:b.punctDwg.left,top:b.punctDwg.top},d&&(a=b.rendEng.getGlyph("dot").clone().set({left:c.left+b.punctWidth,top:c.top,opacity:.6}),e.modify.push(a)));c=p.clone().set({left:c.left,top:c.top,opacity:.6});e.modify.push(c);b.punctDwg&&b.rendEng.canvas.remove(b.punctDwg);b.punctDwg=b.rendEng.draw(e,{group:!0,selectable:!1,repaint:!0})[0]};e(!0);b.rendEng.canvas.observe("mouse:move",function(a){a=
b.rendEng.canvas.getPointer(a.e);b.punctDwg.left=a.x-b.punctDwg.currentWidth/4;b.punctDwg.top=a.y-b.punctDwg.currentHeight/4;b.rendEng.repaint()});b.rendEng.canvas.observe("mouse:up",function(a){var e={x:b.punctDwg.left,y:b.punctDwg.top};a=b.page.getClosestSystem(e);var c=new Toe.Model.SquareNoteNeume,e=a.getSystemSnapCoordinates(e,b.punctDwg.currentWidth),p=e.x-b.punctDwg.currentWidth/2,k=e.y-b.punctDwg.currentHeight/2;c.setBoundingBox([p,k,p+b.punctDwg.currentWidth,k+b.punctDwg.currentHeight]);
var e=a.calcPitchFromCoords(e),p=e.pname,k=e.oct,e={pname:p,oct:k},m=[];d&&(m.push(new Toe.Model.Ornament("dot",{form:"aug"})),e.dotform="aug");m=new Toe.Model.SquareNoteNeumeComponent(p,k,{type:"punctum",ornaments:m});c.addComponent(m);m=new Toe.View.NeumeView(b.rendEng,b.page.documentType);new Toe.Ctrl.NeumeController(c,m);m=a.addNeume(c);if(1==m){var l=b.page.getPreviousSystem(a);l&&b.handleUpdatePrevCustos(p,k,l)}p=b.getOutputBoundingBox([c.zone.ulx,c.zone.uly,c.zone.lrx,c.zone.lry]);e.ulx=p[0];
e.uly=p[1];e.lrx=p[2];e.lry=p[3];if(m+1<a.elements.length)e.beforeid=a.elements[m+1].id;else if(a=b.page.getNextSystem(a))e.beforeid=a.id;$.post(b.apiprefix+"/insert/neume",e,function(a){c.id=JSON.parse(a).id}).error(function(){this.showAlert("Server failed to insert neume. Client and server are not synchronized.")})});$("#chk_dot").bind("click.insert",function(){d=d?!1:!0;e(!1)})};
...
var d=null,e=null;b.rendEng.canvas.observe("mouse:move",function(a){a=b.rendEng.canvas.getPointer(a.e);e=b.page.getClosestSystem(a);var h={strokeWidth:4,opacity:.6};switch(d){case "div_small":a.y=e.zone.uly;if(!b.divisionDwg){var c=e.zone.uly-e.delta_y/2,p=e.zone.uly+e.delta_y/2,k=a.x;b.divisionDwg=b.rendEng.createLine([k,c,k,p],h);b.rendEng.draw({fixed:[b.divisionDwg],modify:[]},{selectable:!1,opacity:.6})}break;case "div_minor":a.y=e.zone.uly+(e.zone.lry-e.zone.uly)/2;b.divisionDwg||(c=e.zone.uly+
e.delta_y/2,p=c+2*e.delta_y,k=a.x,b.divisionDwg=b.rendEng.createLine([k,c,k,p],h),b.rendEng.draw({fixed:[b.divisionDwg],modify:[]},{selectable:!1,opacity:.6}));break;case "div_major":a.y=e.zone.uly+(e.zone.lry-e.zone.uly)/2;b.divisionDwg||(c=e.zone.uly,p=e.zone.lry,k=a.x,b.divisionDwg=b.rendEng.createLine([k,c,k,p],h),b.rendEng.draw({fixed:[b.divisionDwg],modify:[]},{selectable:!1,opacity:.6}));break;case "div_final":if(a.y=e.zone.uly+(e.zone.lry-e.zone.uly)/2,!b.divisionDwg){var c=e.zone.uly,p=e.zone.lry,
k=a.x,m=a.x+b.punctWidth,k=b.rendEng.createLine([k,c,k,p],h),h=b.rendEng.createLine([m,c,m,p],h);b.divisionDwg=b.rendEng.draw({fixed:[k,h],modify:[]},{group:!0,selectable:!1,opacity:.6})[0]}}h=a.x-b.divisionDwg.currentWidth/2;c=a.x+b.divisionDwg.currentWidth/2;e.elements[0]instanceof Toe.Model.Clef&&h<=e.elements[0].zone.lrx?a.x=e.elements[0].zone.lrx+b.divisionDwg.currentWidth/2+1:h<=e.zone.ulx&&(a.x=e.zone.ulx+b.divisionDwg.currentWidth/2+1);e.custos&&c>=e.custos.zone.ulx?a.x=e.custos.zone.ulx-
b.divisionDwg.currentWidth/2-3:c>=e.zone.lrx&&(a.x=e.zone.lrx-b.divisionDwg.currentWidth/2-3);b.divisionDwg.left=a.x;b.divisionDwg.top=a.y;b.rendEng.repaint()});b.rendEng.canvas.observe("mouse:up",function(a){var h=e.getSystemSnapCoordinates({x:b.divisionDwg.left,y:b.divisionDwg.top},b.divisionDwg.currentWidth),c=new Toe.Model.Division(d);a=h.x-b.divisionDwg.currentWidth/2;h=h.y-b.divisionDwg.currentHeight/2;h=[a,h,a+b.divisionDwg.currentWidth,h+b.divisionDwg.currentHeight];c.setBoundingBox(h);a=
new Toe.View.DivisionView(b.rendEng);new Toe.Ctrl.DivisionController(c,a);a=e.addDivision(c);h=b.getOutputBoundingBox(h);h={type:c.key.slice(4),ulx:h[0],uly:h[1],lrx:h[2],lry:h[3]};a+1<e.elements.length?h.beforeid=e.elements[a+1].id:(a=b.page.getNextSystem(e),h.beforeid=a.id);$.post(b.apiprefix+"/insert/division",h,function(a){c.id=JSON.parse(a).id}).error(function(){this.showAlert("Server failed to insert division. Client and server are not synchronized.")})});$("#rad_small").bind("click.insert",
...
function(a){a=b.rendEng.canvas.getPointer(a.e);b.systemDrawing.left=a.x;b.systemDrawing.top=a.y;b.rendEng.repaint()});b.rendEng.canvas.observe("mouse:up",function(a){function d(){var a={ordernumber:e.orderNumber,systemid:e.systemId};null!=h&&(a.nextsbid=h.id);$.post(b.apiprefix+"/insert/systembreak",a,function(a){for(e.setID(JSON.parse(a).id);null!=h;)b.postSystemBreakEdit(h.id,h.orderNumber),h=b.page.getNextSystem(h)}).error(function(){this.showAlert("Server failed to insert system break.  Client and server are not synchronized.")})}
a=b.systemDrawing.left-Math.round(b.systemDrawing.currentWidth/2);var f=b.systemDrawing.top-Math.round(b.systemDrawing.currentHeight/2),e=new Toe.Model.SquareNoteSystem([a,f,a+b.systemDrawing.currentWidth,f+b.systemDrawing.currentHeight]);a=new Toe.View.SystemView(b.rendEng);new Toe.Ctrl.SystemController(e,a);e.setOrderNumber($("#system_number_slider").val());b.page.addSystem(e);b.updateInsertSystemSubControls();var h=b.page.getNextSystem(e);a=b.getOutputBoundingBox([e.zone.ulx,e.zone.uly,e.zone.lrx,
e.zone.lry]);a={pageid:b.page.getID(),ulx:a[0],uly:a[1],lrx:a[2],lry:a[3]};$.post(b.apiprefix+"/insert/system",a,function(a){e.setSystemID(JSON.parse(a).id);d()}).error(function(){this.showAlert("Server failed to insert system.  Client and server are not synchronized.")})})}};
...
b.rendEng.canvas.getPointer(a.e);var f=0,h=0;"c"==d?f=b.clefDwg.currentWidth/4:(f=b.clefDwg.currentWidth/8,h=b.clefDwg.currentHeight/8);b.clefDwg.left=a.x-f;b.clefDwg.top=a.y+h;b.rendEng.repaint()});b.rendEng.canvas.observe("mouse:up",function(a){var f={x:b.clefDwg.left,y:b.clefDwg.top};"f"==d&&(f.x-=b.clefDwg.currentWidth/8,f.y-=b.clefDwg.currentHeight/8);a=b.page.getClosestSystem(f);var h=a.getSystemSnapCoordinates(f,b.clefDwg.currentWidth),f=Math.round((a.zone.uly-h.y)/(a.delta_y/2)),c=new Toe.Model.Clef(d,
{systemPos:f}),p=h.x-b.clefDwg.currentWidth/2,h=h.y-b.clefDwg.currentHeight/2;c.setBoundingBox([p,h,p+b.clefDwg.currentWidth,h+b.clefDwg.currentHeight]);p=new Toe.View.ClefView(b.rendEng);new Toe.Ctrl.ClefController(c,p);p=a.addClef(c);f=a.props.numLines+f/2;h=b.getOutputBoundingBox([c.zone.ulx,c.zone.uly,c.zone.lrx,c.zone.lry]);f={shape:d,line:f,ulx:h[0],uly:h[1],lrx:h[2],lry:h[3]};p+1<a.elements.length?f.beforeid=a.elements[p+1].id:(p=b.page.getNextSystem(a),f.beforeid=p.id);p=a.getPitchedElements({neumes:!0,
custos:!1});0<p.length&&a.getActingClefByEle(p[0])==c&&(h=b.page.getPreviousSystem(a))&&b.handleUpdatePrevCustos(p[0].components[0].pname,p[0].components[0].oct,h);f.pitchInfo=$.map(a.getPitchedElements({clef:c}),function(a){if(a instanceof Toe.Model.Neume){var c=[];$.each(a.components,function(a,b){c.push({pname:b.pname,oct:b.oct})});return{id:a.id,noteInfo:c}}if(a instanceof Toe.Model.Custos){var d=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);$.post(b.apiprefix+"/move/custos",
{id:a.id,ulx:d[0],uly:d[1],lrx:d[2],lry:d[3]}).error(function(){this.showAlert("Server failed to move custos. Client and server are not synchronized.")})}});$.post(b.apiprefix+"/insert/clef",{data:JSON.stringify(f)},function(a){c.id=JSON.parse(a).id}).error(function(){this.showAlert("Server failed to insert clef. Client and server are not synchronized.")})});$("#rad_doh").unbind("click");$("#rad_fah").unbind("click");$("#rad_doh").bind("click.insert",function(){if(!$(this).hasClass("active")){var a=
{left:-50,top:-50};b.clefDwg&&(b.rendEng.canvas.remove(b.clefDwg),a={left:b.clefDwg.left,top:b.clefDwg.top});a=b.rendEng.getGlyph("c_clef").clone().set($.extend(a,{opacity:.6}));b.clefDwg=b.rendEng.draw({fixed:[],modify:[a]},{opacity:.6,selectable:!1,repaint:!0})[0];d="c"}});$("#rad_fah").bind("click.insert",function(){if(!$(this).hasClass("active")){var a={left:-50,top:-50};b.clefDwg&&(b.rendEng.canvas.remove(b.clefDwg),a={left:b.clefDwg.left,top:b.clefDwg.top});a=b.rendEng.getGlyph("f_clef").clone().set($.extend(a,
{opacity:.6}));b.clefDwg=b.rendEng.draw({fixed:[],modify:[a]},{opacity:.6,selectable:!1,repaint:!0})[0];d="f"}});$("#rad_doh").trigger("click")};
Toe.View.SquareNoteInteraction.prototype.handleUpdatePrevCustos=function(a,b,d){var e=d.custos;if(e){e.setRootNote(a,b);var f=d.getActingClefByEle(e);e.setRootSystemPos(d.calcSystemPosFromPitch(a,b,f));f=this.getOutputBoundingBox([e.zone.ulx,e.zone.uly,e.zone.lrx,e.zone.lry]);$.post(this.apiprefix+"/move/custos",{id:e.id,pname:a,oct:b,ulx:f[0],uly:f[1],lrx:f[2],lry:f[3]}).error(function(){this.showAlert("Server failed to move custos. Client and server are not synchronized.")})}else{var h=new Toe.Model.Custos(a,
b),e=d.zone.lrx-gui.punctWidth/2,f=d.zone.uly;h.setBoundingBox([e,f,e+gui.punctWidth,f+gui.punctHeight]);e=new Toe.View.CustosView(gui.rendEng);new Toe.Ctrl.CustosController(h,e);d.setCustos(h);f=this.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]);a={id:h.id,pname:a,oct:b,ulx:f[0],uly:f[1],lrx:f[2],lry:f[3]};if(d=gui.page.getNextSystem(d))a.beforeid=d.id;$.post(this.apiprefix+"/insert/custos",a,function(a){h.id=JSON.parse(a).id}).error(function(){this.showAlert("Server failed to insert custos. Client and server are not synchronized.")})}};
Toe.View.SquareNoteInteraction.prototype.deleteActiveSelection=function(a){toDelete={clefs:[],nids:[],dids:[],cids:[],systemIdArray:[],systemBreakIdArray:[]};var b=[],d=function(b,c){var d=b.eleRef,f=d.system,e=f.getPreviousClef(d);f.removeElementByRef(d);var h=null;c||(f.updatePitchedElements(e),f=f.getPitchedElements(d),h=$.map(f,function(b){if(b instanceof Toe.Model.Neume){var c=[];$.each(b.components,function(a,b){c.push({pname:b.pname,oct:b.oct})});return{id:b.id,noteInfo:c}}if(b instanceof Toe.Model.Custos){var d=
a.getOutputBoundingBox([b.zone.ulx,b.zone.uly,b.zone.lrx,b.zone.lry]);$.post(a.apiprefix+"/move/custos",{id:b.id,ulx:d[0],uly:d[1],lrx:d[2],lry:d[3]}).error(function(){this.showAlert("Server failed to move custos. Client and server are not synchronized.")})}}));toDelete.clefs.push({id:d.id,pitchInfo:h});a.rendEng.canvas.remove(b)},e=function(b){var c=b.eleRef,d=c.system.getPitchedElements({neumes:!0,custos:!1});c.system.removeElementByRef(c);toDelete.nids.push(c.id);a.rendEng.canvas.remove(b);if(1==
d.length)(c=a.page.getPreviousSystem(c.system))&&c.custos&&(c.custos.eraseDrawing(),c.removeElementByRef(c.custos),$.post(a.apiprefix+"/delete/custos",{ids:c.custos.id}).error(function(){this.showAlert("Server failed to delete custos. Client and server are not synchronized.")}),c.custos=null);else if(c==d[0]&&(c=a.page.getPreviousSystem(c.system))&&c.custos){b=c.custos;var f=d[1],d=f.components[0].pname,f=f.components[0].oct,e=c.getActingClefByEle(b),c=c.calcSystemPosFromPitch(d,f,e);b.pname=d;b.oct=
f;b.setRootSystemPos(c);c=a.getOutputBoundingBox([b.zone.ulx,b.zone.uly,b.zone.lrx,b.zone.lry]);$.post(a.apiprefix+"/move/custos",{id:b.id,pname:d,oct:f,ulx:c[0],uly:c[1],lrx:c[2],lry:c[3]}).error(function(){this.showAlert("Server failed to move custos. Client and server are not synchronized.")})}},f=function(b){var c=b.eleRef;c.system.removeElementByRef(c);toDelete.dids.push(c.id);a.rendEng.canvas.remove(b)},h=function(b){var c=b.eleRef;c.system.removeElementByRef(c);c.system.custos=null;toDelete.cids.push(c.id);
a.rendEng.canvas.remove(b)},c=function(c){var l=c.eleRef;toDelete.systemBreakIdArray.push(l.id);toDelete.systemIdArray.push(l.systemId);var g=0;for(doneRemovingElements=!1;!doneRemovingElements;)if(0===l.elements.length||g>=l.elements.length)doneRemovingElements=!0;else{var k=l.elements[g],n=k.view.drawing;k instanceof Toe.Model.Clef?d(n,!0):k instanceof Toe.Model.Neume?e(n):k instanceof Toe.Model.Division?f(n):k instanceof Toe.Model.Custos?h(n):g++}l=a.page.removeSystem(l);b=b.concat(l);a.rendEng.canvas.remove(c)},
p=a.rendEng.canvas.getActiveObject();if(p)p.eleRef instanceof Toe.Model.Clef&&p.eleRef.system.elements[0]!=p.eleRef?d(p,!1):p.eleRef instanceof Toe.Model.Neume?e(p):p.eleRef instanceof Toe.Model.Division?f(p):p.eleRef instanceof Toe.Model.Custos?h(p):p.eleRef instanceof Toe.Model.System&&c(p),a.rendEng.repaint();else if(p=a.rendEng.canvas.getActiveGroup())$.each(p.getObjects(),function(a,b){b.eleRef instanceof Toe.Model.Clef&&b.eleRef.system.elements[0]!=b.eleRef?d(b,!1):b.eleRef instanceof Toe.Model.Neume?
e(b):b.eleRef instanceof Toe.Model.Division?f(b):b.eleRef instanceof Toe.Model.Custos&&h(b)}),a.rendEng.canvas.discardActiveGroup(),a.rendEng.repaint();0<toDelete.nids.length&&$.post(a.apiprefix+"/delete/neume",{ids:toDelete.nids.join(",")}).error(function(){this.showAlert("Server failed to delete neume. Client and server are not synchronized.")});0<toDelete.dids.length&&$.post(a.apiprefix+"/delete/division",{ids:toDelete.dids.join(",")}).error(function(){this.showAlert("Server failed to delete division. Client and server are not synchronized.")});
0<toDelete.cids.length&&$.post(a.apiprefix+"/delete/custos",{ids:toDelete.cids.join(",")}).error(function(){this.showAlert("Server failed to delete custos. Client and server are not synchronized.")});0<toDelete.clefs.length&&$.post(a.apiprefix+"/delete/clef",{data:JSON.stringify(toDelete.clefs)}).error(function(){this.showAlert("Server failed to delete clef. Client and server are not synchronized.")});0<toDelete.systemIdArray.length&&a.postSystemDelete(toDelete.systemIdArray);0<toDelete.systemBreakIdArray.length&&
a.postSystemBreakDelete(toDelete.systemBreakIdArray);if(0<b.length){var k=[];$.each(b,function(a,b){-1===$.inArray(b,k)&&k.push(b)});for(c=0;c<b.length;c++)a.postSystemBreakEditOrder(b[c].id,b[c].orderNumber)}};Toe.View.SquareNoteInteraction.prototype.postSystemBreakEditOrder=function(a,b){$.post(this.apiprefix+"/modify/systembreak",{sbid:a,ordernumber:b}).error(function(){this.showAlert("Server failed to modify system break.  Client and server are not synchronized.")})};
Toe.View.SquareNoteInteraction.prototype.postSystemDelete=function(a){$.post(this.apiprefix+"/delete/system",{sids:a.join(",")}).error(function(){this.showAlert("Server failed to delete system.  Client and server are not synchronized.")})};Toe.View.SquareNoteInteraction.prototype.postSystemBreakDelete=function(a){$.post(this.apiprefix+"/delete/systembreak",{sbids:a.join(",")}).error(function(){this.showAlert("Server failed to delete system break.  Client and server are not synchronized.")})};
Toe.View.SquareNoteInteraction.prototype.postModifySystemZone=function(a,b,d,e,f){$.post(this.apiprefix+"/update/system/zone",{sid:a,ulx:b,uly:d,lrx:e,lry:f}).error(function(){this.showAlert("Server failed to update system zone.  Client and server are not synchronized.")})};Toe.View.SquareNoteInteraction.prototype.handleDelete=function(a){a.data.gui.deleteActiveSelection(a.data.gui)};
Toe.View.SquareNoteInteraction.prototype.handleEventObjectModified=function(a){switch(a.target.eleRef.constructor){case Toe.Model.SquareNoteSystem:a.target.eleRef.controller.modifyWidth(a.target.currentWidth),this.postModifySystemZone(a.target.eleRef.systemId,Math.floor(a.target.eleRef.zone.ulx/this.page.scale),Math.floor(a.target.eleRef.zone.uly/this.page.scale),Math.floor(a.target.eleRef.zone.lrx/this.page.scale),Math.floor(a.target.eleRef.zone.lry/this.page.scale)),a=a.target.eleRef.getLooseElements(),
0<a.length&&(a=$.map(a,function(a,d){return a.view.drawing}),a=new fabric.Group(a),this.rendEng.canvas.deactivateAll(),this.rendEng.canvas.setActiveGroup(a),this.deleteActiveSelection(this))}};
...
!1),$("#btn_neumify_liquescence").toggleClass("disabled",!1));0<d?$("#btn_ungroup").toggleClass("disabled",!1):$("#btn_ungroup").toggleClass("disabled",!0)};Toe.View.SquareNoteInteraction.prototype.getOutputBoundingBox=function(a){gui=this;return $.map(a,function(a){return Math.round(a/gui.page.scale)})};
...
Toe.Model.SquareNotePage.prototype.loadMei=function(a,b){var d=this,e=$($(a).find("page")[0]).attr("xml:id");this.setID(e);var f=$(a).find("surface")[0],h=$("clef, sb",a),c=[];$(h).each(function(a,b){$(b).is("sb")&&c.push(a)});var p=$("neume, sb",a),k=[];$(p).each(function(a,b){$(b).is("sb")&&k.push(a)});var m=$("division, sb",a),l=[];$(m).each(function(a,b){$(b).is("sb")&&l.push(a)});var g=$("custos, sb",a),q=[];$(g).each(function(a,b){$(b).is("sb")&&q.push(a)});$("sb",a).each(function(e,t){var u=
$(t).attr("systemref"),r=$($(a).find("system[xml\\:id="+u+"]")[0]).attr("facs"),r=$(f).find("zone[xml\\:id="+r+"]")[0],r=d.parseBoundingBox(r),v=new Toe.Model.SquareNoteSystem(r);v.setID($(t).attr("xml:id"));v.setOrderNumber(parseInt($(t).attr("n")));v.setSystemID(u);0==e&&b.calcScaleFromSystem(v,{overwrite:!0});u=new Toe.View.SystemView(b);new Toe.Ctrl.SystemController(v,u);d.addSystem(v);$(h).slice(c[e]+1,c[e+1]).each(function(a,c){var e=$(c).attr("shape"),g=parseInt($(c).attr("line")),g=2*-(v.props.numLines-
g),h=$(c).attr("facs"),h=$(f).find("zone[xml\\:id="+h+"]")[0],h=d.parseBoundingBox(h),e=new Toe.Model.Clef(e,{systemPos:g});e.setID($(c).attr("xml:id"));e.setBoundingBox(h);g=new Toe.View.ClefView(b);new Toe.Ctrl.ClefController(e,g);v.addClef(e)});$(p).slice(k[e]+1,k[e+1]).each(function(a,c){var e=new Toe.Model.SquareNoteNeume,g=$(f).find("zone[xml\\:id="+$(c).attr("facs")+"]")[0],g=d.parseBoundingBox(g);e.neumeFromMei(c,g);g=new Toe.View.NeumeView(b,d.documentType);new Toe.Ctrl.NeumeController(e,
g);v.addNeume(e)});$(m).slice(l[e]+1,l[e+1]).each(function(a,c){var e=$(f).find("zone[xml\\:id="+$(c).attr("facs")+"]")[0],e=d.parseBoundingBox(e),g="div_"+$(c).attr("form"),g=new Toe.Model.Division(g);g.setBoundingBox(e);g.setID($(c).attr("xml:id"));e=new Toe.View.DivisionView(b);new Toe.Ctrl.DivisionController(g,e);v.addDivision(g)});$(g).slice(q[e]+1,q[e+1]).each(function(a,c){var e=$(f).find("zone[xml\\:id="+$(c).attr("facs")+"]")[0],e=d.parseBoundingBox(e),g=$(c).attr("pname"),h=parseInt($(c).attr("oct")),
g=new Toe.Model.Custos(g,h);g.setBoundingBox(e);g.setID($(c).attr("xml:id"));e=new Toe.View.CustosView(b);new Toe.Ctrl.CustosController(g,e);v.setCustos(g)})})};Toe.Model.SquareNoteSystem=function(a,b){Toe.Model.System.call(this,a,b)};Toe.Model.SquareNoteSystem.prototype=new Toe.Model.System;Toe.Model.SquareNoteSystem.prototype.constructor=Toe.Model.SquareNoteSystem;
Toe.Model.SquareNoteSystem.prototype.getPitchedElements=function(a){var b={clef:null,neumes:!0,custos:!0};$.extend(b,a);if(b.clef){a=[];for(var d=$.inArray(b.clef,this.elements)+1;d<this.elements.length&&!(this.elements[d]instanceof Toe.Model.Clef);d++){var e=this.elements[d];(e instanceof Toe.Model.Neume&&b.neumes||e instanceof Toe.Model.Custos&&b.custos)&&a.push(e)}return a}return $.grep(this.elements,function(a){if(a instanceof Toe.Model.Neume&&b.neumes||a instanceof Toe.Model.Custos&&b.custos)return a})};
Toe.Model.SquareNoteSystem.prototype.calcPitchFromSystemPos=function(a,b){var d=a-b.props.systemPos,e=Toe.neumaticChroma.length,f=$.inArray(b.shape,Toe.neumaticChroma),h=(f+d)%e;0>h&&(h+=e);var c=Toe.neumaticChroma[h],p=$.inArray("c",Toe.neumaticChroma),e=$.truncateFloat(d/e);0<d&&h>=p&&h<f?e++:0>d&&(h<p||h>f)&&e--;d=4;"f"==b.shape&&(d=3);return{pname:c,oct:d+e}};
...
if(b.objMoving){(a=b.rendEng.canvas.getActiveObject())||(a=b.rendEng.canvas.getActiveGroup());if(a){var e=[];a.eleRef?e.push(a):$.each(a.objects,function(a,b){e.push(b)});$.each(e,function(a,d){var c=d.eleRef;if(c instanceof Toe.Model.Neume){var p={x:d.left,y:d.top},k=b.page.getClosestSystem(p),p=k.getSystemSnapCoordinates(p,d.currentWidth,{ignoreEle:c,y:!1}),m=p.x-d.currentWidth/2,l=p.y-d.currentHeight/2,g=m+d.currentWidth,q=l+d.currentWidth;c.setBoundingBox([m,l,g,q]);$(c).trigger("vEraseDrawing");
c.system.removeElementByRef(c);p=k.addNeume(c);1==e.length&&$(c).trigger("vSelectDrawing");m=b.getOutputBoundingBox([m,l,g,q]);c={id:c.id,ulx:m[0],uly:m[1],lrx:m[2],lry:m[3],pitchInfo:null};p+1<k.elements.length?c.beforeid=k.elements[p+1].id:(k=b.page.getNextSystem(k),c.beforeid=k.id);$.post(b.apiprefix+"/move/neume",{data:JSON.stringify(c)}).error(function(){b.showAlert("Server failed to move neume. Client and server are not synchronized.")})}});1<e.length&&b.rendEng.canvas.discardActiveGroup();
...
Toe.View.CheironomicInteraction.prototype.handleNeumify=function(a){var b=a.data.gui;a=a.data.modifier;var d=b.rendEng.canvas.getActiveGroup();if(d){var e=[],f=null;$.each(d.getObjects(),function(a,b){b.eleRef instanceof Toe.Model.Neume&&(f||(f=b.eleRef.system),b.eleRef.system==f&&e.push(b))});if(!(2>e.length)){e.sort(function(a,b){return a.eleRef.zone.ulx-b.eleRef.zone.ulx});var h=new Toe.Model.CheironomicNeume({modifier:a});numPunct=0;var c=[],p=Number.MAX_VALUE,k=Number.MAX_VALUE,m=Number.MIN_VALUE,
l=null;$.each(e,function(a,e){$.merge(h.components,e.eleRef.components);c.push(e.eleRef.id);var g=d.left+e.left,n=d.top+e.top;if(0<a){var q=0;n<l?q=1:n>l&&(q=-1);h.components[numPunct].relativePitch=q}l=n;p=Math.min(p,g-e.currentHeight/2);k=Math.min(k,n-e.currentHeight/2);m=Math.max(m,n+e.currentHeight/2);numPunct+=e.eleRef.components.length;f.removeElementByRef(e.eleRef);b.rendEng.canvas.remove(e)});h.setBoundingBox([p,k,p+numPunct*b.punctWidth,m]);var g=new Toe.View.NeumeView(b.rendEng,b.page.documentType);
new Toe.Ctrl.NeumeController(h,g);f.addNeume(h);var g=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]),q=h.typeid,n=$.map(h.components,function(a){return a.props.type});a=JSON.stringify({nids:c.join(","),typeid:q,liquescence:a,headShapes:n,ulx:g[0],uly:g[1],lrx:g[2],lry:g[3]});$.post(b.apiprefix+"/neumify",{data:a},function(a){h.id=JSON.parse(a).id}).error(function(){b.showAlert("Server failed to neumify selected neumes. Client and server are not synchronized.")});b.rendEng.canvas.discardActiveGroup();
...
!1,e=function(a){var e={modify:[],fixed:[]},c=null,p=b.rendEng.getGlyph("punctum");a?c={left:-50,top:-50}:(c={left:b.punctDwg.left,top:b.punctDwg.top},d&&(a=b.rendEng.getGlyph("dot").clone().set({left:c.left+b.punctWidth,top:c.top+b.punctHeight/2,opacity:.6}),e.modify.push(a)));c=p.clone().set({left:c.left,top:c.top,opacity:.6});e.modify.push(c);b.punctDwg&&b.rendEng.canvas.remove(b.punctDwg);b.punctDwg=b.rendEng.draw(e,{group:!0,selectable:!1,repaint:!0})[0]};e(!0);b.rendEng.canvas.observe("mouse:move",
function(a){a=b.rendEng.canvas.getPointer(a.e);b.punctDwg.left=a.x-b.punctDwg.currentWidth/4;b.punctDwg.top=a.y-b.punctDwg.currentHeight/4;b.rendEng.repaint()});b.rendEng.canvas.observe("mouse:up",function(a){var e={x:b.punctDwg.left,y:b.punctDwg.top};a=b.page.getClosestSystem(e);var c=new Toe.Model.CheironomicNeume,p=a.getSystemSnapCoordinates(e,b.punctDwg.currentWidth,{y:!1}),e=p.x-b.punctDwg.currentWidth/2,p=p.y-b.punctDwg.currentHeight/2;c.setBoundingBox([e,p,e+b.punctDwg.currentWidth,p+b.punctDwg.currentHeight]);
e={};p=[];d&&(p.push(new Toe.Model.Ornament("dot",{form:"aug"})),e.dotform="aug");p=new Toe.Model.CheironomicNeumeComponent({type:"punctum",ornaments:p});c.addComponent(p);c.typeid=c.name="punctum";p=new Toe.View.NeumeView(b.rendEng,b.page.documentType);new Toe.Ctrl.NeumeController(c,p);var p=a.addNeume(c),k=b.getOutputBoundingBox([c.zone.ulx,c.zone.uly,c.zone.lrx,c.zone.lry]);e.ulx=k[0];e.uly=k[1];e.lrx=k[2];e.lry=k[3];if(p+1<a.elements.length)e.beforeid=a.elements[p+1].id;else if(a=b.page.getNextSystem(a))e.beforeid=
...
1){var p=this;$(f).each(function(a,b){var e="punctum";"true"==$(this).parent().attr("quilisma")?e="quilisma":"virga"==d||"bivirga"==d||"trivirga"==d?e="virga":"tractulus"==d?e="tractulus":"gravis"==d?e="gravis":"oriscus"==d?e="oriscus":"stropha"==d&&(e="stropha");var f=[],h=$("> dot",this).attr("form");h&&f.push(new Toe.Model.Ornament("dot",{form:h}));e=new Toe.Model.CheironomicNeumeComponent({type:e,ornaments:f});0<a&&(e.relativePitch=parseInt(c.edges[a-1]));p.addComponent(e)});this.deriveName();
...
Toe.Model.CheironomicPage.prototype.loadMei=function(a,b){var d=this,e=$(a).find("surface")[0],f=$(a).find("section")[0],h=$(f).children(),c=[];$(h).each(function(a,b){$(b).is("sb")&&c.push(a)});c.push(h.length);var p=0;$(c).each(function(a,m){var l=$(h).slice(p,m),g=new Toe.Model.CheironomicSystem([0,0,0,0],{numLines:0});g.setOrderNumber(parseInt($(m).attr("n")));var q=null,q=0==a?$(f).attr("xml:id"):$($(h)[c[a-1]]).attr("xml:id");g.setID(q);q=new Toe.View.SystemView(b);new Toe.Ctrl.SystemController(g,
q);d.addSystem(g);var n=Number.MAX_VALUE,t=0;$(l).each(function(c,f){var h=new Toe.Model.CheironomicNeume,l=$(e).find("zone[xml\\:id="+$(f).attr("facs")+"]")[0],l=d.parseBoundingBox(l);l[1]<n&&(n=l[1]);l[3]>t&&(t=l[3]);h.neumeFromMei(f,l);l=new Toe.View.NeumeView(b,d.documentType);new Toe.Ctrl.NeumeController(h,l);0==a&&0==c&&b.calcScaleFromNeume(h,{overwrite:!0});g.addNeume(h,{justPush:!0})});g.elements.length&&(g.setBoundingBox([g.elements[0].zone.ulx,n,g.elements[g.elements.length-1].zone.lrx,
t]),$(g).trigger("vRenderSystem",[g]));p=m+1})};Toe.Model.CheironomicSystem=function(a,b){Toe.Model.System.call(this,a,b)};Toe.Model.CheironomicSystem.prototype=new Toe.Model.System;Toe.Model.CheironomicSystem.prototype.constructor=Toe.Model.CheironomicSystem;
