...
Toe.View.SquareNoteInteraction.prototype.handleEventObjectModified=function(a){switch(a.target.eleRef.constructor){case Toe.Model.SquareNoteSystem:console.log(a),a.target.eleRef.controller.modifyZone(a.target.getCenterPoint().x,a.target.currentWidth),this.postModifySystemZone(a.target.eleRef.systemId,Math.floor(a.target.eleRef.zone.ulx/this.page.scale),Math.floor(a.target.eleRef.zone.uly/this.page.scale),Math.floor(a.target.eleRef.zone.lrx/this.page.scale),Math.floor(a.target.eleRef.zone.lry/this.page.scale)),
a=a.target.eleRef.getLooseElements(),0<a.length&&(a=$.map(a,function(a,d){return a.view.drawing}),a=new fabric.Group(a),this.rendEng.canvas.deactivateAll(),this.rendEng.canvas.setActiveGroup(a),this.deleteActiveSelection(this))}};
...
Toe.Model.SquareNotePage.prototype.loadMei=function(a,b){var d=this,e=$($(a).find("page")[0]).attr("xml:id");this.setID(e);var f=$(a).find("surface")[0],h=$("clef, sb",a),c=[];$(h).each(function(a,b){$(b).is("sb")&&c.push(a)});var p=$("neume, sb",a),k=[];$(p).each(function(a,b){$(b).is("sb")&&k.push(a)});var m=$("division, sb",a),l=[];$(m).each(function(a,b){$(b).is("sb")&&l.push(a)});var g=$("custos, sb",a),q=[];$(g).each(function(a,b){$(b).is("sb")&&q.push(a)});$("sb",a).each(function(a,e){var u=
$(e).attr("facs"),u=$(f).find("zone[xml\\:id="+u+"]")[0],u=d.parseBoundingBox(u),r=new Toe.Model.SquareNoteSystem(u);r.setID($(e).attr("xml:id"));r.setOrderNumber(parseInt($(e).attr("n")));r.setSystemID($(e).attr("xml:id"));0==a&&b.calcScaleFromSystem(r,{overwrite:!0});u=new Toe.View.SystemView(b);new Toe.Ctrl.SystemController(r,u);d.addSystem(r);$(h).slice(c[a]+1,c[a+1]).each(function(a,c){var e=$(c).attr("shape"),g=parseInt($(c).attr("line")),g=2*-(r.props.numLines-g),h=$(c).attr("facs"),h=$(f).find("zone[xml\\:id="+
h+"]")[0],h=d.parseBoundingBox(h),e=new Toe.Model.Clef(e,{systemPos:g});e.setID($(c).attr("xml:id"));e.setBoundingBox(h);g=new Toe.View.ClefView(b);new Toe.Ctrl.ClefController(e,g);r.addClef(e)});$(p).slice(k[a]+1,k[a+1]).each(function(a,c){var e=new Toe.Model.SquareNoteNeume,g=$(f).find("zone[xml\\:id="+$(c).attr("facs")+"]")[0],g=d.parseBoundingBox(g);e.neumeFromMei(c,g);g=new Toe.View.NeumeView(b,d.documentType);new Toe.Ctrl.NeumeController(e,g);r.addNeume(e)});$(m).slice(l[a]+1,l[a+1]).each(function(a,
c){var e=$(f).find("zone[xml\\:id="+$(c).attr("facs")+"]")[0],e=d.parseBoundingBox(e),g="div_"+$(c).attr("form"),g=new Toe.Model.Division(g);g.setBoundingBox(e);g.setID($(c).attr("xml:id"));e=new Toe.View.DivisionView(b);new Toe.Ctrl.DivisionController(g,e);r.addDivision(g)});$(g).slice(q[a]+1,q[a+1]).each(function(a,c){var e=$(f).find("zone[xml\\:id="+$(c).attr("facs")+"]")[0],e=d.parseBoundingBox(e),g=$(c).attr("pname"),h=parseInt($(c).attr("oct")),g=new Toe.Model.Custos(g,h);g.setBoundingBox(e);
g.setID($(c).attr("xml:id"));e=new Toe.View.CustosView(b);new Toe.Ctrl.CustosController(g,e);r.setCustos(g)})})};Toe.Model.SquareNoteSystem=function(a,b){Toe.Model.System.call(this,a,b)};Toe.Model.SquareNoteSystem.prototype=new Toe.Model.System;Toe.Model.SquareNoteSystem.prototype.constructor=Toe.Model.SquareNoteSystem;
