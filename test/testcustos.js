(function() {
    ...
    test("Constructor", function() {
        ...
        equal(cModel.oct, 3);
        equal(cModel.rootStaffPos, null);
        deepEqual(cModel.zone, new Object());
        equal(cModel.id, null);
        equal(cModel.staff, null);
    ...
    test("Set Staff", function() {
        ...
        cModel.setStaff(sModel);

        equal(cModel.staff, sModel);

        // test object is not a staff
        raises(function() {
            cModel.setStaff(new Object());
        });
    });
    ...
    test("Set Root Staff Pos", function() {
        ...
        equal(custosModel.rootStaffPos, -2);
        equal(custosModel.pname, "a");
        equal(custosModel.oct, 3);

        custosModel.setRootStaffPos(-4);

        equal(custosModel.rootStaffPos, -4);
